...
int borrow_book(char buffer[]) {
    ...
    while (fgets(line, sizeof(line), input_file) != NULL) {
        ...
        strcpy(current_line, line); // Preserve original line
        char *token2 = strtok(line, ",");
        char current_book_id[20];
        strcpy(current_book_id, token2);
...
int add_member(char buffer[]) {
    ...
    char sep[2] = "\n";
    char data[400];
    strcpy(data, buffer + 2); // Extract user data from buffer (skip the prefix)

    // Check if the extracted data is empty or invalid
    if (strlen(data) == 0 || strchr(data, ',') == NULL) { // Ensure valid "username,password" format
        pthread_mutex_unlock(&logins_mutex);
        return 0; // Failure due to invalid or empty input
    }

    FILE* fd = fopen("logins.txt", "a");
    if (fd == NULL) { // Handle file open errors
        pthread_mutex_unlock(&logins_mutex);
        return 0;
    }

    fseek(fd, 0, SEEK_END); // Move to the end of the file
    strcat(data, sep);      // Add newline character
    int data_written = fwrite(data, sizeof(char), strlen(data), fd);

    fclose(fd);
    pthread_mutex_unlock(&logins_mutex);

    if (data_written == strlen(data)) // Success if all characters are written
        return 1;
    else
        return 0;
}
